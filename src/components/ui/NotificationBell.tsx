  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 rounded-full border border-[#0a0a0a]"></span>
        )}
      </button>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 mt-2 w-80 sm:w-96 bg-[#111] border border-white/10 shadow-2xl rounded-2xl z-50 overflow-hidden"
            >
              ...
              <div className="p-3 border-b border-white/10 bg-black/20">
                <div className="relative">
                  <SearchIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 w-3.5 h-3.5 text-white/40" />
                  <input
                    type="text"
                    placeholder="Search notifications..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full bg-white/5 border border-white/10 rounded-lg py-1.5 pl-9 pr-3 text-xs text-white placeholder:text-white/30 focus:outline-none focus:border-primary/50 transition-colors"
                  />
                </div>
              </div>

              <div className="flex gap-2 p-3 overflow-x-auto border-b border-white/10 no-scrollbar">
                ...
                {filteredNotifications.length === 0 ? (
                  ...
                  filteredNotifications.map((n) => (
                    <div
                      ...
                      className={`p-4 border-b border-white/5 cursor-pointer transition-colors flex gap-4 ${
                        n.read ? "bg-transparent opacity-60" : "bg-white/[0.02] hover:bg-white/[0.04]"
                      }`}
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-start mb-1">
                          <h4 className={`text-sm font-medium ${n.read ? "text-white/70" : "text-white"}`}>
                            {n.title}
                          </h4>
              ...
              <div className="p-3 border-t border-white/10 bg-black/40 text-center">
                 <button className="text-[10px] uppercase tracking-wider text-white/50 hover:text-white flex items-center justify-center w-full gap-2 transition-colors font-bold">
